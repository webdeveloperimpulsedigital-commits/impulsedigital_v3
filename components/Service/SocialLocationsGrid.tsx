...
export const SocialLocationsGrid: React.FC<{ currentLocation?: string }> = ({ currentLocation }) => {
  const [isOpen, setIsOpen] = useState(false);
  const dropdownRef = useRef<HTMLDivElement>(null);
  
  const locationsToShow = socialMediaLocations.filter(
        ...
        <div className="loc-dropdown-container" ref={dropdownRef}>
          ...
          <div className={`loc-dropdown-list ${isOpen ? 'open' : ''}`} data-lenis-prevent="true">
            {locationsToShow.map((loc) => (
              <Link
                key={loc.slug}
                href={`/brand-infrastructure/social-media-marketing/${loc.slug}/`}
